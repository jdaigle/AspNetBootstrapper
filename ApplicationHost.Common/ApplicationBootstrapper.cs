...
namespace ApplicationHost
{
    public static class ApplicationBootstrapper
    {
        ...
        public static void Bootstrap()
        {
            try
            {
                ...
                lock (locker)
                {
                    ...
                    {
                        var bootstrapper = (IBootrapper)Activator.CreateInstance(item);
                        bootstrapper.Init();
                    }
        ...
        private static IEnumerable<Type> ScanAssembliesForEndpoints(IEnumerable<Assembly> assemblies)
        {
            ...
            {
                foreach (var type in assembly.GetTypes().Where(t => typeof(IBootrapper).IsAssignableFrom(t) && t != typeof(IBootrapper)))
                {
                    yield return type;
                }
            }
